...
def about_section(content, image_path=None, social_links=None):
    ...
    # Profile Image
    if image_path:
        st.image(image_path, use_column_width=False, width=170)
    
    # Image Upload
    uploaded_file = st.file_uploader("Upload profile picture", type=['png', 'jpg', 'jpeg'])
    if uploaded_file is not None:
        st.image(uploaded_file, use_column_width=False, width=170)
...
def team_about_section(title, description, team_members=None):
    st.markdown(f"""
