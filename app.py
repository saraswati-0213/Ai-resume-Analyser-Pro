...
class ResumeApp:
    ...
    def render_ats_gauge(self, score):
    ...
    def render_skills_gap(self, analysis, role_info):

        st.markdown("---")
        st.subheader("📈 Skills Gap Analysis")

        required_skills = [skill.lower() for skill in role_info["required_skills"]]

        resume_skills = analysis.get("skills", [])

        if resume_skills and isinstance(resume_skills[0], dict):
            resume_skills = [skill.get("name", "").lower() for skill in resume_skills]
        else:
    ...
    def render_career_roadmap(self, analysis, role_info):
        ...
        if skills and isinstance(skills[0], dict):
            skills = [skill.get("name", "").lower() for skill in skills]
        else:
            skills = [str(skill).lower() for skill in skills]
        ...
        if "python" not in skills:
            beginner.append("Python")
 
        if "git" not in skills:
            beginner.append("Git & GitHub")

        if "sql" not in skills:
            intermediate.append("SQL")

        if "rest api" not in skills:
            intermediate.append("REST APIs")

        if "django" not in skills:
            intermediate.append("Django / Flask")

        if "docker" not in skills:
            advanced.append("Docker")

        if "aws" not in skills:
            advanced.append("AWS")

        if "ci/cd" not in skills:
            advanced.append("CI/CD")

        st.success(f"🎯 Target Role : {role_info['description']}")
...
✔️ Apply for Internships & Jobs
""")

        total_missing = len(beginner) + len(intermediate) + len(advanced)

        progress = max(0, min(100, 100 - total_missing * 10))

        st.progress(progress / 100)

        st.success(f"🎯 Career Readiness : {progress}%")
    def generate_analysis_pdf(self, analysis):
        ...
        story.append(Paragraph("<b>Smart Resume AI - Analysis Report</b>", styles["Title"]))

        story.append(Paragraph(f"ATS Score : {analysis.get('ats_score',0)}", styles["Normal"]))
    ...
    def render_strength_weakness(self, analysis):
        st.markdown("---")
        st.subheader("💪 Resume Strength & Weakness")
    ...
    # ATS Score
        ...
        if ats >= 80:
            ...
            weaknesses.append("Improve ATS Score (Target: 80+)")
    ...
    # Projects
        ...
        if len(projects) > 0:
            strengths.append("Projects Section Added")
        else:
            weaknesses.append("Add Academic or Personal Projects")
    ...
    # Experience
        ...
        if len(experience) > 0:
            ...
            weaknesses.append("Add Internship / Training Experience")
    ...
    # GitHub
        ...
        for item in weaknesses:
            st.write(f"❌ {item}")
    def render_resume_rating(self, analysis):
        ...
        if score >= 90:
            ...
            st.write("✅ MNCs")
            st.write("✅ High Paying Jobs")

        elif score >= 75:
            st.write("✅ Internships")
            st.write("✅ Service Companies")
            st.write("✅ Fresher Jobs")

        else:
            st.write("✔️ Improve Resume")
            st.write("✔️ Add More Projects")
            st.write("✔️ Improve Skills")

        st.progress(score / 100)

        st.caption(f"Overall Resume Score : {score}/100")
    def render_interview_questions(self, skills):
        st.subheader("🤖 AI Interview Questions")

        questions = self.interview_generator.generate_questions(skills)

        tab1, tab2, tab3, tab4 = st.tabs(
        ["Technical", "HR", "Project", "Coding"]
    )
...
font-weight:600;
    ...
    def add_footer(self):
        ...
        with col2:
            ...
            <b>AI Resume Analyzer Pro</b><br>
            Powered by <b>Python</b>, <b>Streamlit</b> & <b>Google Gemini AI</b>
            </p>

            <p style='text-align:center; font-size:14px;'>
            Developed By <b>Saraswati Tanwar</b>
            </p>
        ...
        if uploaded_file is not None:
            try:
                if uploaded_file.type == "application/pdf":
                    resume_text = self.resume_parser.extract_text_from_pdf(uploaded_file)
                else:
                    resume_text = self.resume_parser.extract_text_from_docx(uploaded_file)
    ...
    current_name.replace(
        ...
        st.toast("Check out these repositories: [30-Days-Of-Rust](https://github.com/tanwardiksha27)", icon="ℹ️")
    def render_about(self):
        """Render the about page using clean Streamlit components"""
        import base64
        import os
        ...
        st.subheader("A powerful AI-driven platform for optimizing your resume")
        st.write("---")

        # Profile Section
        st.header("Saraswati Tanwar")
        st.caption("Computer Science Student | Python Developer | AI & ML Enthusiast")
        
        # Display Profile Image cleanly
        if image_base64:
            st.markdown(f'<img src="{image_base64}" style="width:180px; height:180px; border-radius:50%; object-fit:cover; border:3px solid #4CAF50; display:block; margin-bottom:15px;">', unsafe_allow_html=True)
        else:
            st.markdown('<img src="https://avatars.githubusercontent.com/tanwardiksha27" style="width:180px; height:180px; border-radius:50%; object-fit:cover; border:3px solid #4CAF50; display:block; margin-bottom:15px;">', unsafe_allow_html=True)

        # Bio Text
        st.write("""
        Hello! I'm *Saraswati Tanwar*, a Computer Science student passionate about
        Python, Artificial Intelligence, and Full Stack Development.
        I developed AI Resume Analyzer Pro to help students and job seekers
        analyze their resumes, improve ATS scores, and prepare for interviews
        using AI-powered insights.
        """)
        
        # Simple Social Links text (No messy raw HTML strings)
        st.markdown("[📧 Email Me](mailto:tanwarsaraswati087@gmail.com)")
        st.write("---")
            ...
            if uploaded_file:
                ...
                if analyze_standard:
                    with st.spinner("Analyzing your document..."):
                        ...
                        # ATS Gauge
                        self.render_ats_gauge(analysis["ats_score"])
                        ...
                        self.render_interview_questions(skills)

                        # Strength & Weakness
                        self.render_strength_weakness(analysis)
                        self.render_skills_gap(analysis, role_info)
                        self.render_resume_rating(analysis)
                        self.render_career_roadmap(analysis, role_info)

                        pdf_file = self.generate_analysis_pdf(analysis)
    ...
    def render_home(self):
        ...
        "An intelligent resume analysis platform that evaluates resumes, calculates ATS score, identifies missing skills, and provides personalized career recommendations using Artificial Intelligence."
)
        # Features Section
        ...
        st.markdown("---")
    st.subheader("📊 Why Choose Smart Resume AI?")
    ...
    with col4:
        ...
        with col2:
            if st.button("Start Analysis", key="get_started_btn", 
...
"""
    ...
    def main(self):
        ...
        with st.sidebar:
            animation = self.load_lottie_url(
                "https://assets5.lottiefiles.com/packages/lf20_xyadoh9h.json"
)

            if animation:
                st_lottie(animation, height=200, key="sidebar_animation")
